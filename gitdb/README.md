# gitdb — query a git repo with SQL

`gitdb` exposes a git repository as a set of read-only **SQLite virtual tables**
...
```
gitdb [flags] [repo] [sql]
```

- `repo` is a local path, a clone URL, or an `owner/repo` GitHub shorthand
  (default `.`).
- `sql`, if given, is run and the results printed; otherwise gitdb reads SQL
...
gitdb --example top-authors BurntSushi/ripgrep

# your own SQL, positional form
gitdb BurntSushi/ripgrep "SELECT count(*) FROM commits WHERE is_merge"

# pipe a multi-statement script
echo "SELECT name FROM refs WHERE is_branch;" | gitdb .
...
| `-cache <dir>` | cache directory (default `<user cache>/gitdb`) |
...
Run `gitdb -schema` to see the exact `CREATE TABLE` for each.
...
the binary). List them with `gitdb -list-examples` and run one with
`gitdb --example <name> <repo>`:
...
`gitdb --example search-content BurntSushi/ripgrep` (which is just
...
`gitdb --example file-history BurntSushi/ripgrep` (most recent rows):
...
`gitdb --example big-commits BurntSushi/ripgrep`:
...
- Remote repos are bare-cloned to `<cache>/gitdb/repos/<host>/<owner>/<repo>`
...
5. **Bounded rename detection.** Content-based rename detection is O(adds ×
   ...
   (unlimited) a full `git/git` churn **did not finish in 5 minutes**. gitdb
...
`gitdb` is discovered as a Go app from its `go.mod`. Changes to this directory
