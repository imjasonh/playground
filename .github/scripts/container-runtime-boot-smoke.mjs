// Boot a container2wasm runtime directory in headless Chromium and prove the
...
// the emulator is tens of megabytes and is deliberately not in git, so CI's
// simulator build has no runtime to boot. Chromium is not WebKit, but it
// exercises the same page, the same cross-origin-isolated origin, and the same
// wasm — so a failure here is a real failure, and a pass leaves only
// "does WebKit agree" to check on a device.
//
...
//
// Usage: node container-runtime-boot-smoke.mjs <htdocs-dir> [--arch aarch64] [--timeout 1200]
...
try {
  ...
  // Ask the guest something only a booted kernel can answer. Paste rather than
  // synthesised keystrokes: it goes straight into xterm's data path, so the
  // test does not depend on where focus happens to be.
  await page.evaluate(() => window.containerLabTerminal.paste("uname -sm\r"));
  await waitForGuest(new RegExp(`Linux ${arch}`), `"Linux ${arch}"`);
...
async function waitForGuest(pattern, description) {
  ...
  throw new Error(`timed out after ${timeoutSeconds}s waiting for ${description}`);
}
