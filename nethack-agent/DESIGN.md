...
Play uses `grok-4.6` unless you pass `--model`. After the run it prints the token cost the Cursor SDK reported for that play. If the SDK has not reported a cost yet, the log says so. The harness does not compute a price from token counts. That number is for the person watching the run. It is not a score, and it is not shown to the agent.
