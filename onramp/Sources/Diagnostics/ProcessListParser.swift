...
enum ProcessListParser {
    ...
    static func summarize(
        ...
        if pctOfRAM >= 15 || totalMB >= 2048 {
            ...
            fixes.append(
                String(
                    format: "Memory use for “%@” looks moderate (%.0f MB). If it’s still slow, check CPU in a moment, disk/network waits, or that app’s own diagnostics — Onramp can re-check after you reproduce the slowness.",
                    query,
                    totalMB
                )
            )
        ...
        fixes.append("Onramp only reports usage — it does not quit or kill processes for you.")
