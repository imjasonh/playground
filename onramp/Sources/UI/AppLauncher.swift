...
enum AppLauncher {
    ...
    /// Handles Onramp custom URLs and System Settings deep links.
    static func handleOpenURL(_ url: URL) -> Bool {
        if handleOnrampURL(url) { return true }
        if SettingsDeepLinks.open(url) { return true }
        return false
    }

    /// Handles `onramp://open-app/Activity%20Monitor`.
    static func handleOnrampURL(_ url: URL) -> Bool {
        guard url.scheme == "onramp" else { return false }
...
enum ActivityMonitorLinks {
    static let markdownURL = "onramp://open-app/Activity%20Monitor"
...
/// Applies all Onramp markdown deep-link rewrites.
