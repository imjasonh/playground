...
enum ToolboxCheck: String, CaseIterable, Identifiable, Hashable {
    ...
    case currentWifi
    case arpNeighbors
    // Once-online / advanced (demoted in UI)
    case processUsage
    case topMemory
    case topCPU
    case diskSpace
    ...
    /// Checks that matter when you can't reach the Internet.
    static var networkCases: [ToolboxCheck] {
        [
            .pathStatus, .defaultRoute, .interfaces, .dnsConfig, .dnsLookup, .dnsTrace,
            .reachability, .ping, .traceroute, .httpProbe, .proxyConfig, .vpnInterfaces,
            .hostsFile, .currentWifi, .arpNeighbors,
        ]
    }

    /// Useful after you're online (or for chat) — not the product focus.
    static var advancedCases: [ToolboxCheck] {
        [
            .processUsage, .topMemory, .topCPU, .diskSpace, .memoryPressure, .systemLoad,
            .powerAssertions, .listeningPorts, .crashReports, .loginItems, .userStorage,
            .batteryPower,
        ]
    }

    var isNetwork: Bool { Self.networkCases.contains(self) }
