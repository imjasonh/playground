...
2. **Apple Watch companion** (`io.github.imjasonh.playground.watch`) — glanceable
   ...
   session; the phone therefore re-calls `startWatchApp` every ~45s while
   riding, and the Watch recovers/restarts a lost session. The session collects
   heart rate, active/basal energy, Watch cycling distance, and
