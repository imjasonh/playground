...
// manifest escape the cache directory. We keep this strict on purpose
// — anything weird should be a load error, not a partial fetch.
// Pasta does not attempt to be a general remote-dependency security
// boundary (SSRF / host policy); git + the user's environment own that.
