...
| Manual / e2e (optional in CI) | `docker run --rm $(node-image build -L …)` when Docker is available (auto-skip otherwise; prints a daemon **tag**, not `@sha256`, because load does not populate `RepoDigests`) |
