...
func RunJoin(ctx context.Context, ch io.ReadWriter, hub *Hub, keyFP, userID string) {
	t := newTerm(ch)
	if userID == "" {
		runJoinNew(ctx, ch, t, hub, keyFP)
		return
	}
	runJoinManage(t, userID, keyFP)
}

func runJoinNew(ctx context.Context, ch io.ReadWriter, t *term, hub *Hub, keyFP string) {
...
func runJoinManage(t *term, userID, keyFP string) {
	...
	t.Printf("Try: ssh menu@<host> or ssh fortune@<host>\n")
}
