...
function drawHex(context, terrain, cell, camera, origin, hoverKey, brushKeys) {
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    const midX = (screen[0].x + screen[1].x + screen[2].x + screen[3].x) / 4;
    const midY = (screen[0].y + screen[1].y + screen[2].y + screen[3].y) / 4;
    const center = project(
      axialToWorld(q, r, size).x,
      mean * camera.heightScale,
      axialToWorld(q, r, size).z,
      camera,
      origin,
    );
    if (midY < center.y - 2) {
  ...
  const centroidWorld = {
    x: axialToWorld(q, r, size).x,
    y: mean * camera.heightScale,
    z: axialToWorld(q, r, size).z,
  };
  for (let i = 0; i < 6; i += 1) {
    const next = (i + 1) % 6;
    const normal = flipToward(
      faceNormal(topWorld[i], topWorld[next], centroidWorld),
      0,
      1,
      0,
    );
    const color = shade(grassColor(mean, slope, wet), normal);
    context.fillStyle = rgb(hovered || inBrush ? mix(color, [255, 236, 160], 0.28) : color);
    fillPoly(context, [top[i], top[next], project(centroidWorld.x, centroidWorld.y, centroidWorld.z, camera, origin)]);
    context.fill();
  }

  context.strokeStyle = hovered ? "rgba(48, 36, 14, 0.7)" : "rgba(36, 56, 28, 0.22)";
  ...
  if (hexIsUnderwater(terrain, q, r)) {
    ...
    context.fill();
    context.strokeStyle = "rgba(210, 236, 242, 0.28)";
