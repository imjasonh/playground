...
- **OTA is signed GHCR, not a raw URL.** Layer media type
  ...
  slot. If `:latest`'s layer digest matches `ota/last_digest` or the
  running slot, log `ota: no updates` and do not download. Otherwise
  verify the Cosign signature (Fulcio leaf + identity allowlist)
