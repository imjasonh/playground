...
use inkbot_esp32::{default_ota_repo, known_ota_app, require_https_url, OTA_APP_INKBOT};
...
pub const DEFAULT_OTA_APP: &str = OTA_APP_INKBOT;
...
impl AppConfig {
    ...
    pub fn load(partition: EspDefaultNvsPartition) -> Result<Option<Self>> {
        ...
        let (ota_app, ota_repo, ota_tag, ota_poll_secs) = load_ota(partition)?;
...
fn load_ota(partition: EspDefaultNvsPartition) -> Result<(String, String, String, u64)> {
    let nvs = match EspNvs::new(partition, NVS_OTA_NS, false) {
        Ok(n) => n,
        Err(_) => {
            return Ok((
                DEFAULT_OTA_APP.into(),
                default_ota_repo(DEFAULT_OTA_APP),
                DEFAULT_OTA_TAG.into(),
                DEFAULT_OTA_POLL_SECS,
            ));
        }
    };
    let app = match read_str(&nvs, NVS_OTA_NS, "app", 32)? {
        Some(s) if known_ota_app(&s) => s,
        Some(s) => {
            log::warn!("ota/app={s} is unknown; using {DEFAULT_OTA_APP}");
            DEFAULT_OTA_APP.into()
        }
        None => DEFAULT_OTA_APP.into(),
    };
    let repo = read_str(&nvs, NVS_OTA_NS, "repo", 256)?
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| default_ota_repo(&app));
