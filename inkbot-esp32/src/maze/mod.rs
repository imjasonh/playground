...
pub use render::{
    crop_packed, dirty_rect, layout_for, render_empty, render_progress, ByteRect, Layout,
};
pub use solve::solve;
...
/// Default maze size; 40×24 cells fill an 800×480 panel with ~3 px walls
/// (~19 px pitch, ~4 mm on the 7.5″ module).
pub const MAZE_COLS: u16 = 40;
pub const MAZE_ROWS: u16 = 24;

/// Delay between solution updates (and the pause on the empty maze).
/// Pair with [`CELLS_PER_TICK`] — for example 10 cells every 1000 ms.
pub const TICK_MS: u64 = 5000;

/// How many solution cells to add on each partial refresh.
pub const CELLS_PER_TICK: usize = 1;
