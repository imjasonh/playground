...
| `deploy-sshapp.yml` | push to `main` touching `sshapp/**`, manual | **App-specific:** Terraform apply for the GKE Autopilot sshapp stack (ko rebuild + roll). Skips with a notice until repo vars/secrets for WIF are set (`SSHAPP_WIF_PROVIDER`, `SSHAPP_DEPLOY_SA`, `SSHAPP_TF_STATE_BUCKET`, `SSHAPP_TFVARS`); see `sshapp/README.md` |
