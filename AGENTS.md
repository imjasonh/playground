...
| `container-runtime.yml` | pushes touching `ios/ContainerRuntime/**` or its CI scripts, manual | **App-specific:** builds the wasm container runtime the iOS *Container Lab* experiment hosts. One leg converts `alpine` to a WASI wasm and boots it under `wasmtime`; the other converts `arm64v8/alpine` with `c2w --to-js` (QEMU Wasm + Emscripten), uploads it as an artifact for `ios/ContainerRuntime/fetch-runtime.sh`, and boots it in headless Chromium (`.github/scripts/container-runtime-boot-smoke.mjs`) to prove the guest reaches a shell. Needs Docker + buildx and takes hours, so it is not part of `test.yml`, and its multi-megabyte output is never committed |
| `wasm-hello.yml` | push to `main` touching `wasm-hello/**`, manual | **App-specific:** builds `wasm-hello` for wasip1 and pushes it to GHCR as an OCI artifact (`ghcr.io/<owner>/<repo>/wasm-hello`) via the app's own `cmd/publish` (go-containerregistry), so the iOS *Wasm Service* experiment has something to pull. Uses the built-in `GITHUB_TOKEN`; a newly created package is private until someone makes it public once |
