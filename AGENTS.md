...
| `container-runtime.yml` | pull requests touching `ios/ContainerRuntime/**`, manual | **App-specific:** builds the wasm container runtime the iOS *Container Lab* experiment hosts. One leg converts `alpine` to a WASI wasm and boots it under `wasmtime`; the other converts `arm64v8/alpine` with `c2w --to-js` (QEMU Wasm + Emscripten) and uploads it as an artifact for `ios/ContainerRuntime/fetch-runtime.sh`. Needs Docker + buildx and takes hours, so it is not part of `test.yml`, and its multi-megabyte output is never committed |
