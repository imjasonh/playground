...
| `deploy-exe.yml` | push to `main`, manual | Deploys changed [exe.dev](https://exe.dev) apps (top-level dirs whose `iac/` declares the `benjamin-lykins/exedev` Terraform provider). Uses `ko_build` to push images to GHCR, applies Terraform with state in Cloudflare R2 (`playground-terraform-state`), and authenticates to exe.dev by minting `EXEDEV_TOKEN` from the `EXEDEV_SSH_PRIVATE_KEY` secret. Also needs `TF_STATE_R2_ACCESS_KEY_ID`, `TF_STATE_R2_SECRET_ACCESS_KEY`, and the existing Cloudflare secrets to create the state bucket. Manual dispatch redeploys every exe app. |
...
4. Document required repo secrets in the app README
   (`EXEDEV_SSH_PRIVATE_KEY`, R2 state keys; Cloudflare secrets are shared).
