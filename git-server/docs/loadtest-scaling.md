...
4, max 24). On the Worker, fan-out uses the `SELF` service binding (see
`wrangler.toml`); public-URL self-fetch is blocked with Cloudflare error 1042.
Cloudflare also caps a request chain at **32 Worker invocations** (coordinator
+ each service-binding call), so the shard max stays under that — `shards=32`
used to throw a bare HTTP 500.
